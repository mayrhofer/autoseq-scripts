...
##Below: for Markus looping/debugging. Comment from here for use in production setting.
load('~/snpTable.Rdata')
myOrder=c(55L, 54L, 59L, 7L, 6L, 14L, 12L, 2L, 3L, 4L, 20L, 21L, 27L, 
          74L, 19L, 73L, 30L, 22L, 29L, 52L, 84L, 1L, 68L, 5L, 48L, 15L, 
          13L, 47L, 103L, 80L, 75L, 43L, 39L, 64L, 60L, 90L, 53L, 85L, 
          99L, 95L, 97L, 35L, 31L, 86L, 8L, 23L, 91L, 69L, 56L, 57L, 58L, 
          49L, 50L, 51L, 36L, 37L, 38L, 81L, 82L, 83L, 32L, 33L, 34L, 44L, 
          45L, 46L, 108L, 61L, 62L, 63L, 9L, 10L, 11L, 24L, 25L, 26L, 92L, 
          93L, 94L, 28L, 17L, 110L, 102L, 104L, 96L, 107L, 106L, 79L, 77L, 
          41L, 66L, 114L, 113L, 105L, 112L, 101L, 115L, 98L, 116L, 100L, 
          111L, 88L, 71L, 109L, 16L, 18L, 76L, 78L, 40L, 42L, 65L, 67L, 
          87L, 89L, 70L, 72L)

setwd('~/ALASCCAdata/testSamples')
dirs=dir(path = 'dirs',recursive = F,pattern = 'AL-P-000')[myOrder]
patients=NULL; for (i in 1:length(dirs)) patients[i]=substr(strsplit(dirs[i],'-')[[1]][3],4,9)
sampleData=data.table(n=NA,dirs,tcov=NA,tcov10=NA,ncov=NA,ncov10=NA,mapd1=NA,snps=NA,muts=NA,snvmuts=NA,maf=NA,
                      pik3ca=NA,pik3r1=NA,pten=NA,ptenloss=NA)
allMuts=NULL; writeJson=TRUE #<------ if skip json files in testing
snpPos=NULL
for (sample in 1:length(dirs)) {
  files=dir(path = paste('dirs',dirs[sample],sep='/'),full.names = T,recursive = T)
  ...
  opts <- getopt(args,opt = c("--cnr", files[grep(pattern = '.cnr',x = files)],
                              ...
                              "--somaticvcf", paste('vcf.02/',n,'.vcf',sep=''),
                              #"--somaticvcf", files[grep(pattern = '.vardict-somatic.vep.vcf.gz',x = files)][1],
                              "--png", paste(n,"plot.png",sep='.'),
                              "--json.cna", paste('jsons/json.cna',n,"json",sep='.'),
                              "--json.purity", paste('jsons/json.purity',n,"json",sep='.'),
                              "--chrsizes", "~/human_g1k_v37_decoy.chrsizes.txt") )
  # comment this far for use in production, + last few lines of script.
  ...
  #------------------------------------------------------------
  try( {
    png(filename = paste(dirs[sample],n,'png',sep='.'),width=11.7,height=8.3,units="in",res=300)
  ...
  #below: used only for Markus' testing (for loop), remove in production setting
  if (!is.null(alf)) {
    sampleData$tcov[sample]=round(median(alf$td))
    sampleData$ncov[sample]=round(median(alf$nd))
    sampleData$tcov10[sample]=round(quantile(alf$td,0.1))
    sampleData$ncov10[sample]=round(quantile(alf$nd,0.1))
    sampleData$snps[sample]=nrow(alf)
    sampleData$muts[sample]=nrow(salf)
    snpPos=rbind(snpPos,alf[,1:2])
  }
  sampleData$mapd[sample]=mapd1
  sampleData$snvmuts[sample]=sum(salf$type=='snv')
  if (sum(salf$t>=0.05)>0) sampleData$maf[sample]=round(median(salf$t[salf$t>=0.05]),3)
  ...
  allMuts=rbind(allMuts,salf)
  
  rm(alf,salf,bins,segments,smoothedAi,g,vcf,vcf2,purity)
}
